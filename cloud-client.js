    ...
    create(config) {
      ...
      return {
        ...
        async signUp(email, password, invitationCode = "") {
          ...
          return assertResult(await client.auth.signUp({
            ...
            options: {
              ...
              data: normalizedInvitationCode ? { invitation_code: normalizedInvitationCode } : {}
            }
          }));
        },
        ...
        async deleteAccount() {
          ...
          for (let index = 0; index < paths.length; index += 1e3) {
            assertResult(
              await client.storage.from(FEEDBACK_BUCKET).remove(paths.slice(index, index + 1e3))
            );
          }
        ...
        async loadAdminFeedback(status = null, limit = 100, offset = 0) {
          ...
          const paths = [...new Set(
            (result.items ?? []).flatMap((item) => Array.isArray(item.imagePaths) ? item.imagePaths : [])
          )];
          const urlsByPath = /* @__PURE__ */ new Map();
          if (paths.length) {
          ...
          return {
            ...
            items: (result.items ?? []).map((item) => ({
              ...item,
              images: (item.imagePaths ?? []).map((path) => ({ path, url: urlsByPath.get(path) })).filter((image) => image.url)
            }))
          };
        },
