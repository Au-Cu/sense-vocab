...
- 反馈提交在数据库侧按账户串行计数，并限制为每小时 20 次、每天 60 次，避免并发绕过和存储滥用。
...
- 后台合规台账按问题保存风险等级、处理状态、证据索引、下一步方案和追加式历史快照；台账用于审计和人工判断，不自动形成法律意见。
...
- 学习卡保持专注，不常驻展示发音和例句的来源长文本；普通用户可从“更多 > 条款、隐私与内容来源”进入无需登录的公共来源目录。
- 公共目录可按单词、稳定内容 ID、作者、许可证和来源类型查询需要署名的运行时音频与例句，并把重复使用的同一资产合并展示，同时保留它关联的全部稳定内容引用。
- 构建会自动核对运行时可追溯资产的作者、许可证、许可证链接、直接来源和修改说明，并把公共目录、第三方声明与许可证文本一同打包；内部证据哈希、审核流程和风险明细继续留在权利台账及管理员合规看板。
- 公开署名迁移不改变既有内容身份或权利结论。权利尚未确认的历史内容继续保持阻断，不因学习卡不再显示警告而视为已获授权或可收费发行。
