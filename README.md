...
一个按义项学习英语词汇的网页应用。当前发布版本：`1.3.0`。

## 1.3.0 当前发布要点

- 单词卡片展开后再次点击即可进入用户自建的“易混词球体”，支持搜索添加易混词、自动维护双向关系、拖动/双指旋转，以及从球面单词直接跳回对应词卡。
- 单词卡片与易混词球体之间的切换改为连续缩放动画；返回时会回到最初进入球体前的界面上下文，减少闪烁、字号跳变和迷路感。
- 单词列表新增“熟悉程度”筛选，可与现有排序同时生效；手机端筛选栏按整行布局。复习/新学/强化阶段的进度统计按各自阶段单独计算，强化阶段不再隐藏已掌握义项。
- 新手教程在计划设置步骤允许直接取消并安全返回，不会再因为遮罩阻断页面操作。
- “消息通知”中的公告默认只展示标题与日期，正文和图片需要手动展开；后台支持公告置顶与取消置顶，置顶状态会优先展示，并保留管理员审计记录。
- 从 `1.2.x` 升级到 `1.3.0` 时，需要先同步最新 Supabase 迁移再部署前端与后台。该迁移只为既有公告增加置顶字段、排序索引和管理员 RPC，不会批量改写公告已读记录、词书内容或任何用户学习数据。
...
- 登录后每次学习仍会先落盘到本机账户缓存；断网时可继续使用，恢复联网后自动同步。
- 云端采用版本号进行乐观并发控制。另一台设备已经写入更新时，当前设备会先拉取并按义项、日期、学习窗口自动合并，再以新修订号重试；无法安全自动判定时才要求用户选择记录。
...
- 每个浏览器标签页拥有独立版本向量，重新联网、窗口重新获得焦点及定时轮询都会拉取其他设备更新。
...
如果是从 `1.2.x` 升级到 `1.3.0`，这里的最新迁移会为公告系统补充默认关闭的置顶字段、排序索引和管理员置顶 RPC；迁移本身不会批量改写学习记录、义项状态或用户计划。
...
- 背词页左下角的“反馈”会自动关联当前单词；后台反馈记录可直接打开该单词的只读卡片。
...
- 点击单词卡片后显示义项。
- 点击某个义项表示已经熟悉，该义项从本轮收起。
- 每个义项展示词性，如 `n.`、`v.`、`adj.`、`adv.`。
- 点击“下一词”时，未处理义项默认进入待强化队列。
- 每天生成一个学习队列：先放到期复习，再放计划新学单词。
- 未点击的义项会安排到第二天继续复习；点击过的义项视为已熟悉，不再出现。
- 不再人工推进日期；日期始终取真实当天，没学习的日子会让预计完成日期顺延。
- 重置入口可选择重置全部进度或只重置当前单词进度，并且需要二次确认。
