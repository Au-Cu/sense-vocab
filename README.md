...
一个按义项学习英语词汇的网页应用。当前发布版本：`1.3.1`。

## 1.3.1 当前发布要点

- 修复学习记录增长后同步时报 `The quota has been exceeded` 的问题，压缩本地存储布局并收敛多标签页并发元数据。
- 旧游客记录和账户缓存会安全迁移；即使本地容量不足，也会保留当前更新并优先完成云端同步。
- 修复 double check 状态流：从 `待强化` 进入复习并确认后仍需在下一学习日再次确认；当天强化卡片会把该义项显示为绿色已处理且不会重复要求确认。
- 从 `待复习` 进入复习后，确认即转为 `已掌握`；未确认则回退为 `待强化` 并在当天强化阶段重新出现。重置本次标记或重学单词时会同步清除当天临时状态。
- 根据后台反馈补充 `part` 的“分开”、`party` 的“派对”和 `parachute` 的“降落伞”义项，并修正 `participate` 的“参与”和 `order` 的“秩序”释义与例句。
- 新增五个反馈词条的词库身份回归测试，并把学习状态测试扩展到四个连续学习日；发布词库现含 6,607 个单词、10,214 个义项。
- 本版本不需要 Supabase 数据库迁移，不改变云端快照结构，也不会批量改写或迁移现有用户学习数据；既有单词和义项 ID 保持稳定，仅为三个新增义项分配新 ID。
