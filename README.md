...
- 修复学习记录增长后同步时报 `The quota has been exceeded` 的问题。本机状态不再重复保存当前词书，完整考研进度的估算占用由约 `5.29 MB` 降至约 `2.65 MB`。
- 旧游客记录和账户缓存会在读取时自动、原子地迁移为紧凑格式；迁移失败不会先删除旧记录，也不会改动任何义项状态或学习历史。
- 并发版本向量改为按稳定浏览器设备维护，历史标签页产生的向量会自动合并，避免长期使用后同步元数据持续膨胀。
- 本地容量仍不足时，登录账户的更新会优先继续同步到云端；界面会明确提示保持页面开启，不再用底层英文异常中断同步流程。
- 本补丁不需要新增 Supabase 数据库迁移，云端快照结构和 `1.3.0` 的公告置顶能力保持兼容。
